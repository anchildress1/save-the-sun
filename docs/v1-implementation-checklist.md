...
- [ ] Sköll vs Oracle lines attributable to the correct speaker _(Sköll's taunts were cut from the UI — his only on-board line is his Ask; it now speaks in his own first-person predatory voice ("I scent a fire rune on her.") rather than the Oracle's third-person paraphrase, so the speaker is unmistakable — `ux-copy.md` §2)_
...
**Sköll voice fix (S11 follow-up):** his on-board Ask was rendering in the Oracle's register — `Sköll asks after {value-phrase}.`, third-person framing wrapped around the Oracle's own `valuePhrase` grammar (imported straight from `oracle.ts`), so the wolf's box read as the Oracle's line. `skollAskEcho` (`skoll/skoll.ts`) is now self-contained and first-person predatory per the Cast Voice Charter (`ux-copy.md` §2): "I scent a fire rune on her." / "A gold rune. Mine." / "Light or dark — I taste a dark one." / "Sowilo. I name it in the dark." / "Three power. I can smell it." Power 1–6 is spoken as a word (not the Oracle's digit grammar) so the two speakers never collide. The internal Gemini payload still uses `valuePhrase` (model-facing facts, not player copy). [U] `skollAskEcho` per-axis voice + all five power ops + no-exclamation guard (`skoll.test`); the real-output echo assertions (`page.server.test`, `api/action/server.test`) and the component/e2e fixtures track the new line.
