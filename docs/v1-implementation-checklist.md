...
**Implementation (S8):** a per-session **chronological event log** (`src/lib/server/debug/log.ts`) — a `DebugEvent` stream across channels (`turn`, `oracle`, `skoll`, `gemini`, `session`). Lifecycle-linked to the round through `session.ts` (reset on a new round, evicted with the session), bounded, written from the one place every move resolves — the `api/action` route. `turn` events carry the engine's deterministic `truth` beside the `inference` that reached it (the two tagged columns) with a `source` (`gemini`/`floor`) on Sköll's moves; a human **Ask** logs the raw question (`oracle`) then the Oracle's reading beside the engine's answer; a human **Cast** logs the verdict with no inference; a Sköll **Cast** logs immediately, a Sköll **Ask** is parked (decision held on `SkollState.pendingDecision`) and its row lands once the human's reaction reveals the answer — reasoning and engine truth on the same row. His **action** each turn also logs on the `skoll` channel (flagged `warn` + "Floor fired" when the deterministic floor stood in), as does his Scry/Hex/Pass on the human's Ask. The shown reasoning is the grounded `summarizePayload` (his earned facts + sheet, or the opening hunch) — **not** a model thought-trace: `includeThoughts` was tried and reverted (on MINIMAL thinking + structured JSON the trace is off-persona noise). Instead the **raw Gemini request/response** is teed from the move/reaction seams into a sink (`captureGemini`) and drained onto the log as `sensitive` `gemini` events. The round's **secret** opens the log as a `sensitive` `session` event.
...
**Tests landed:** [I] secret session event, raw question + turn tagging, human/Sköll cast rows, floor flag (warn), truth-vs-reasoning on the parked Ask, his reaction logged, raw Gemini I/O drained as sensitive (`action/server.test`) · [U] event store seq/trim/isolation/reset, `debugLevel` env + dev/prod defaults, `filterForLevel` strip/off, the Gemini sink, reasoning threading + `pendingDecision` + `summarizePayload` (`debug/log.test`, `skoll.test`) · [I] `/api/debug` + `/debug` load level-filtering — verbose/demo/off (`api/debug/server.test`) · [C] stream renders turn columns, non-turn message + JSON, sensitive badge, newest-first, empty + off states (`debug/page.svelte.test`). _(The live model's behaviour stays the deferred persona eval; the raw-I/O events are the diagnostic that replaces trusting a thought-trace.)_
