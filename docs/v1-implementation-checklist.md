...
- [ ] Consistency check: an **automated** assertion that, after a mid-round reload, the board marks + voiced line match what the **debug log** shows for the same round _(deferred to v2 — the debug log is env-gated off in prod and unlinked from the game; v1 proves view ↔ round agreement via the [C]/[E] suites, the log ↔ view cross-check stays a manual demo observation. Unchecked to match `test-checklist.md` §10.5 — a manual demo observation is not the automated assertion this box specifies.)_
