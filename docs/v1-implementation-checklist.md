...
**Opener variety + reaction presence (S6 follow-up):** his *opening* Ask was repetitive — on move 1 his payload is byte-identical every round (no facts, fixed board), so a low-thinking model lands on the same first trait every time (the "always gold" tell). `SkollState` now carries a **seeded per-round hunch** (a trait-level colour/element phrase drawn from the crypto seed — varies per round, reproducible, never a rune by name so it can't echo the secret, never light/dark since that clean split is the optimal opener the prompt forbids). It is surfaced to Gemini **only on the opening move** and is kept out of the stringified payload otherwise, so it can't bias him once he reasons from earned facts. Later moves already vary on their own facts, so they get no nudge. Separately, his reaction-consideration gate was raised (**`REACTION_CHANCE` 0.5 → 0.65**) so he shows up to Scry/Hex on ~⅔ of the human's Asks rather than half. _(The opener's live feel — does Gemini actually follow the hunch — stays the deferred persona eval, not a CI gate.)_
...
**Implementation (S8):** a per-session **chronological event log** (`src/lib/server/debug/log.ts`) — a `DebugEvent` stream across channels (`turn`, `oracle`, `skoll`, `gemini`, `session`). Lifecycle-linked to the round through `session.ts` (reset on a new round, evicted with the session), bounded, written from the one place every move resolves — the `api/action` route. The view's axis is **engine fact vs LLM inference**: the rules engine produces facts (the `turn` verdicts, the `session` secret), the LLM actors produce inference (the `oracle` reading the human, the `skoll` move + reasoning, the raw `gemini` I/O). So a **`turn` event carries ONLY the engine's deterministic verdict** — the inference that reached the move is never bolted onto it. A human **Ask** logs the raw question + the Oracle's reading on the `oracle` channel, then the engine's answer on the `turn`; a human/Sköll **Cast** logs the verdict on the `turn`; a Sköll move logs his action + `reasoning` + `source` (`gemini`/`floor`, `warn` + "Floor fired" on the fallback) + this-turn cross-offs on the `skoll` channel, and his **Ask**'s engine answer lands on the `turn` once the human reacts (no parked decision — the reasoning already sits on his `skoll` event). His Scry/Hex/Pass on the human's Ask logs on `skoll` too. The shown reasoning is the grounded `summarizePayload` (earned facts + sheet, or the opening hunch) — **not** a model thought-trace: `includeThoughts` was tried and reverted (on MINIMAL thinking + structured JSON the trace is off-persona noise). Instead the **raw Gemini request/response** is teed from the move/reaction seams into a **per-session sink** (`captureGemini`, scoped by an `AsyncLocalStorage` the route opens — no cross-session bleed) and drained onto the log as `sensitive` `gemini` events; the snapshot is a cycle-safe sanitizer (strips functions, breaks cycles, marker on throw) so neither `json()` nor the load serializer 500s. The round's **secret** opens the log as a `sensitive` `session` event. The view encodes three things independently: the card's **colour is its source** (Human · Oracle · Sköll — including his raw Gemini move/reaction calls, since the model is computing *his* move — · Engine), an **LLM-vs-deterministic badge** marks whether a model call reached it (the Oracle's read and raw Gemini are always LLM; a Sköll move/reaction is LLM only when Gemini decided it, deterministic on the floor; turn verdicts + the secret are deterministic), and a **turn-part chip** (Ask / Cast / React / Round) names the phase. Severity stays a separate badge.
...
**Tests landed:** [I] secret session event, question on `oracle` + verdict on `turn`, human/Sköll cast verdicts, his reasoning/source/cross-offs on `skoll`, floor flag (warn), engine-verdict on a hexed Ask, his reaction logged, per-session raw Gemini I/O drained as sensitive (`action/server.test`) · [U] event store seq/trim/isolation/reset, `debugLevel` env + dev/prod defaults, `filterForLevel` strip/off, the **per-session** Gemini sink (isolation, no-context no-op, cycle/function sanitizer, marker-on-throw), `summarizePayload` (`debug/log.test`, `skoll.test`) · [I] `/api/debug` + `/debug` load level-filtering — verbose/demo/off (`api/debug/server.test`) · [C] turn renders as a single engine-fact card, engine-vs-inference border/chip by channel, non-turn message + JSON, sensitive badge, newest-first, no-overflow, empty + off states (`debug/page.svelte.test`). _(The live model's behaviour stays the deferred persona eval; the raw-I/O events are the diagnostic that replaces trusting a thought-trace.)_
