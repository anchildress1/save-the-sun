...
>
> **Voice has two layers** — see [`architecture.md`](./architecture.md#voice--input-the-live-mic-and-output-delivery). **Output** is mic-independent: every game move is composed server-side and voiced through one shared TTS route (the Oracle and Sköll, a `voice` param) — text always, audio when on — so the board speaks with or without the mic. **Input** is the optional Live mic, reached through the eclipse medallion (tap to wake, tap to sleep). The requirements below hold as shipped; the delivery output layer and the output mute (R11) sit on top, and Sköll's game-move voice rides the shared route (his prebuilt-clip library is reserved for the deferred ambience layer).
