...
- **Header:** title "Save the Sun" + tagline "A race to beat Sköll and save the light," the night-progress indicator, and a turn pill ("Your move." / "Sköll moves.").
...
- **Right column:** the Rite transcript (You / Oracle / Sköll, with interpretation echoes) → the Reactions panel (Scry · Hex) → the "Ask the Oracle" free-text input (the which-axes howto lives in the onboarding popovers, not on the board) → the "Cast the rune" button. No suggestion chips — pre-filled questions would do the player's deduction for them, and the deduction is the game.
...
- **How-to-play guidance:** "Ask. Cross off what it can't be. Cast when you're ready." — delivered through the first-run onboarding / coach-mark popovers (R7), not as a persistent on-board explainer.
...
**Two seeds, kept apart.** The on-screen **board order** is a separate seed from the **secret**, and the two must never be linked. The board-order seed is *display state*: server-generated per load (Web Crypto uint32), public, rendered on screen, and later handed to Sköll so he reasons over the same layout — its only job is to shuffle the grid so element/light-dark groupings don't jump out. The **secret rune is the engine's own**, chosen on a seed it generates and holds; it must never be derived from (or equal to) the public board-order seed, or the visible layout would leak the answer. The crypto board seed isn't a secrecy requirement (the order is public) — it's just a clean RNG path; the secrecy guarantee lives entirely in the engine.

**Legality is about well-formedness, not redundancy.** The engine rejects malformed, mixed-type, out-of-turn, and unknown-rune calls — it does **not** disallow a repeated question. Re-asking is legal play for either side (the Oracle answers the same truth again, the turn is spent). Avoiding redundant questions is the deterministic floor's concern — a move-quality heuristic in its candidate set — not an engine-enforced error.
...
- **Asset delivery pipeline.** Clean up the generated image set into one stable stylized art direction, then generate AVIF/WebP fallbacks, responsive variants, and a checked bundle-size budget so deploy cost and first-load weight stay bounded.
