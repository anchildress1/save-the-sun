...
- [x] Undo the 'don't think' Sköll logic — live wolf (MINIMAL thinking, temp 1) doesn't converge (150+ turn games); raise thinking/lower temp + force a cast at ≤2 candidates. _(the non-convergence was temp 1's rambling, not low thinking — so a server-authoritative `guard` source forces the closing cast once ≤2 runes survive, killing the tail and freeing temperature to be tuned for pace alone: MINIMAL thinking + move temp 0.7 narrows as hunch-y as the in-window floor; tuned against `node scripts/skoll-sim.mjs --live`)_
