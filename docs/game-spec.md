...
- One query type per Ask: element, power, fill/light, hue, or one rune name. Ranges and comparisons are allowed for power ("fewer than 3?", "at least 2?"). Negation is the not-equal (`ne`) operator ("is it not fire?", "isn't it light?") — it flips the predicate, resolved by the engine, never applied by the player; a negated range resolves to the opposite comparison. Refuses mixed-type questions, secret reveals, and instruction overrides. Refused Asks do not consume the turn; a resolved Ask does.
