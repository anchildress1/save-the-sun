...
- [x] **P2 — Build + wire Sköll's voice.** *(landed: `src/lib/voice/skollScript.ts` — the machine-readable taunt library, source of truth shared by the generator and the director; `scripts/skoll-voice.mjs` — the build-time TTS generator (Algieba, director's-notes prompt, retry on the model's 500s, one command regenerates), writing committed `static/audio/skoll/<id>.pcm.b64` clips; `src/lib/voice/skollDirector.ts` — picks a clip on engine events with no-repeat + once-per-stage milestone memory; `delivery.ts` `deliverClip()` — plays a clip through the **same** speaker the Oracle uses, so the two serialize on one queue (R9: he never overlaps her) and the audio toggle's mute gates both; clips are warmed into an in-memory cache by `preloadClips()` the moment audio is enabled (the opt-in gesture), so the first taunt plays with no fetch latency and silent players never download the library. The page (`+page.svelte`) triggers him on his first turn + hunt milestones, a wrong cast, a Hexed Ask, and the player's win; his caption rides his own frame, never the Oracle's panel (R10). Triggers are **engine events only** — idle (a client timer) and the taunt bucket (a spoken input) defer; his Ask and winning-cast `{Rune}` stay text (dynamic). R9 mic isolation: an awake Live mic sleeps before his clip while the two layers still coexist.)* Net-new (S12 was a drafted script only): the clip library is **static assets** (zero runtime TTS, per R8), the director hands clips to the shared `deliver()` queue **as audio** — bypassing the Oracle TTS route (his lines are never synthesized at runtime). Audible during board-only play, mic off. Hard prerequisite for P4: until it lands, retiring Live leaves board-only play with no Sköll voice.
...
| R8 Sköll clips · R9 mic discipline · S12 · S13 | **Landed (P2)** — prebuilt clip library + director on the shared `deliverClip()` queue; engine-event triggers only, **taunt bucket + idle defer to P5/timer**, his dynamic lines stay text. R9 one-speaker holds structurally (one speaker queue) and mic-isolation sleeps an awake Live mic while the layers coexist |
