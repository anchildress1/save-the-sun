...
- **Audio is delivery, not a session.** Every line is composed once and delivered as text (always, for game moves) and as audio when audio is on. The shared speaker queue is **source-agnostic**: a **game move** (the Oracle's answer/refusal, **Sköll's Ask**) is synthesized by the server TTS route in the speaker's voice (cached, key-side; P2 added a `voice` param so the one route serves both characters) and written to its panel/frame (R10). **Ambience** (Sköll's taunt buckets — splash/idle/hunt mood) is a later audio-only layer, prebuilt clips played as-is, **exempt from captioning** by the revised R10 (it carries no game state). Mic-independent either way.
...
| The server TTS route — voices **both** game-move voices (Oracle + Sköll's Ask), one `voice` param, each line shaped by its speaker's director's-notes | Hands-free voice + barge-in — returns as the opt-in Live adapter |
| The prebaked-clip *approach* — reserved for the later **ambience** layer (audio-only, no caption) | Per-game generated Sköll lines — still a future consideration |

> **Sköll's voice split across two layers.** His **game moves** (the Ask now, his winning cast later) speak through the shared TTS route in his voice — landed for the Ask in P2. His **ambience** (the drafted taunt buckets, `ux-copy.md` §2) is a later audio-only layer of prebuilt clips — script drafted, pipeline not built. P4 must not retire Live until board-only play has Sköll's voice; the Ask-via-TTS satisfies that for his game presence.
...
- [x] **P1 — One spoken-line path.** *(landed: `src/lib/voice/delivery.ts` seam, `POST /api/voice/tts` route, the audio toggle. The Oracle's Ask answers and refusals flow through `deliver()`. The wake greeting ("I wake with the fire.") is **gone**: it was a Live session-wake line, and the toggle is a plain sound on/off, not a wake — so it was removed from the codebase entirely, not parked in the allow-list. The **reaction** resolutions (§3), the **cast** outcomes (§4), and the **end-screen outcome** voice too — shared in `$lib/voice/{reactionLines,castLines,outcomeLines}.ts`, composed server-side via `react` / `cast` / `outcome` line descriptors (the scry lines carry the overheard answer's query; the wrong-cast names a board-validated rune), so the route still voices only server-owned lines. The end screen voices one beat of its splash copy — a **win in the Oracle's voice** (the victory coda), a **loss in Sköll's** (the night-everlasting verse) — so the player hears who took the day. Dynamic/dramatized spoken lines (dropping the preset allow-list for a server-authored-line gate) are deferred — see `ttd.md`.)* Two pieces, in order. **(a) The shared speaker queue** — `createSpeaker` already plays base64 PCM in the browser; wrap it as the one `deliver(line)` seam that any audio source feeds (TTS now, prebuilt clips in P2). The queue never synthesizes; it only plays audio + renders the caption. **(b) The server-side TTS route** — the Oracle's audio *source* behind `deliver()`: turns a *server-composed line* into audio, key staying server-side. Net-new (the sole voice route today mints Live tokens; after P4 there is otherwise no key-safe way to voice her text). Abuse guards are not optional — per-session **and** global rate limits, and it voices only **server-owned, allow-listed lines** (engine outcomes, the pre-engine refusals/guards, and the greeting — see Constraints), never arbitrary client text, so it can't be spammed for free Gemini TTS once the token path is gone. Cache the finite, templated Oracle lines so most turns replay a cached clip. Then her answer flows text-box → interpret → engine → line → `deliver()`, and the S6 greeting is the round's first delivered line (no wake event). **Audio is gated behind a user gesture** (Constraints): the audio toggle's enabling tap lands here, not P3, and audio stays off until it inits the speaker. No mic. Collapses `direct()` + `beginInvitation()` once the route exists.
- [x] **P2 — Sköll's game-move voice (his Ask), via the shared TTS route.** *(landed.)* His **Ask** — a recurring game move (R10) — is voiced in his own voice through the **same** server TTS path as the Oracle. **Why the TTS route, not prebuilt clips:** his game-move lines are *dynamic* (the Ask names a sign, his cast a `{Rune}`), which is exactly what server TTS handles; one route avoids a parallel audio system, and his Ask was the actually-missing voice (the prebuilt-clip buckets are *ambience* — a separate audio-only layer, below). Mechanics: `tts.ts` takes a `voice` (cache keyed by voice + text); `lines.ts` adds a `skoll-ask` descriptor that recomposes his line **server-side from the parked query** via `skollAskEcho` (still a server-owned line, never client text) and `voiceForLine` picks `SKOLL_VOICE`; **`synthPrompt` wraps each line in its speaker's director's-notes** so the one model voices both in character — Sköll a deep gravelly growl, the Oracle a brisk reverent weight (a bare line reads flat and generic). The `Advance` wire carries the `query`; the page voices it through `deliver()`, gated on an idle session like her answer. His Ask stays written on his frame (`skoll-echo`, R10). **Deferred:** his winning cast (same route, once the rune rides the wire, with a caption) and the audio-only ambience taunt layer (`ttd.md`).
...
- **Sköll's dynamic game-move lines are voiced through the TTS route, not prebuilt clips.** His Ask names a sign and his winning cast names a `{Rune}` (`ux-copy.md` §2) — dynamic content no single prebuilt clip can carry, but exactly what the server TTS route handles (the server recomposes the line from the engine's structured query, so it stays a server-owned line). The Ask landed in P2; the winning cast follows the same way once the rune rides the wire. Both stay written on his frame (R10). The prebuilt-clip approach is reserved for the **fixed ambience** buckets, which are audio-only.
...
| Spec item | Fate |
|---|---|
| R3 engine parity · R10 every-game-move-written · R11 output mute | **Holds** — delivery-agnostic; one toggle, panel + captions carry it. R10 revised 2026-06-14: game **moves** are always written; **ambience** (Sköll's taunt library) is audio-only flavor, exempt |
