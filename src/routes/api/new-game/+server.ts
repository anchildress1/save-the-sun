...
export const POST: RequestHandler = ({ locals, request, getClientAddress }) => {
	const engine = resetEngine(locals.sessionId);
	// A fresh game is a clean slate — drop this session's spent rate-limit windows so the first call
	// isn't throttled by the game just finished. Per-session only; the global ceiling is untouched.
	resetLimiterKey(
		buildLimiterKey(resolveLimiterAddress(request, getClientAddress), locals.sessionId)
	);
	return json({
